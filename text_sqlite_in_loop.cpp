...
int main() {
    ...
    int check_repetition = sqlite3_table_column_metadata(db, NULL, file_names_array[0].c_str(), NULL, NULL, NULL, NULL, NULL, NULL);

    if (check_repetition != SQLITE_OK)
    {
        fprintf(stderr, "Tables are creating... \n");

        for (int i = 0; i < file_names_array.size(); i++) {
            cout << "*************************************" << endl;
            cout << "\n" + file_names_array[i] << endl;
            cout << "********" << endl;
            ...
            // print the results 
            cout << "lines: " << no_of_rows << " || columns: " << no_of_columns << endl;
            ...
            string wp;
            cout << "If you want to add WP to name of the table please write. If you don't press N: " << endl;
            cin >> wp;
            ...
            if (wp != "N") {
                ...
                tablecode << "CREATE TABLE IF NOT EXISTS " << wp << file_names_array[i] << " (ID INTEGER PRIMARY KEY AUTOINCREMENT," << column_names_in_string << "); ";
                query = tablecode.str();
                ...
                sqlcode.str("");
                sqlcode << "INSERT INTO " << wp << file_names_array[i] << " (" << column_names_bind_in_string << ") VALUES(" << question_marks_in_string << "); ";
                query = sqlcode.str();
                ...
                cout << wp << file_names_array[i] + " ended." << endl;
            }
            else {
    ...
    {
        fprintf(stdout, "Data has already written to DB file. \n");
    }
